...
export class JiraClient {
  ...
  async jiraApiCall(method: string, url: string, body?: any): Promise<any> {
    ...
    try {
      ...
      console.error(
        `❌ Failed to parse JSON response for ${url}: ${jsonError}`
      );
  ...
  async searchIssues(
    ...
    try {
      ...
      console.log(
        `🔍 Found ${response.issues?.length || 0} issues (${
          response.total
        } total)`
      );
  ...
  async getIssue(issueKey: string): Promise<JiraIssue> {
    ...
    try {
      ...
      console.log(
        `📝 Issue summary: ${response.fields?.summary || "No summary"}`
      );
      ...
      if (!response.fields) {
        ...
        const fallbackResponse = await this.jiraApiCall(
          "GET",
          `/rest/api/3/issue/${issueKey}`
        );
        console.log(
          `🔍 Fallback response has fields: ${!!fallbackResponse.fields}`
        );
    ...
    for (const url of urls) {
      ...
      try {
        ...
        if (!metadataResponse.ok) {
          console.warn(
            `Failed to fetch attachment metadata for ${attachmentId}`
          );
          continue;
        }
  ...
  async postImplementationComment(
    ...
    try {
      ...
      const content = JiraFormatter.createImplementationCommentADF(
        claudeOutput,
        taskSummary
      );
      ...
      console.log(
        `✅ Successfully posted implementation comment to ${issueKey}`
      );
    } catch (error) {
      console.warn(
        `Failed to post implementation comment to ${issueKey}: ${error}`
      );
      throw error;
    }
  }
  ...
  async postClarityComment(issueKey: string, assessment: any): Promise<void> {
    try {
      ...
      console.warn(
        `Failed to post clarity assessment to ${issueKey}: ${error}`
      );
      throw error;
    }
  }
  ...
  async transitionIssue(issueKey: string, statusName: string): Promise<void> {
    try {
      ...
      console.log(
        `✅ Successfully transitioned ${issueKey} to "${statusName}"`
      );
    } catch (error) {
      console.warn(
        `Failed to transition ${issueKey} to "${statusName}": ${error}`
      );
      throw error;
    }
  }
  ...
  formatIssueDetails(
    ...
    return JiraExtractor.formatIssueDetails(
      issue,
      comments,
      linkedResources,
      relatedIssues
